...
def save_probabilidades(dic,keys):
    ...
    for i in probabilidades:
        probabilitySet = probabilitySet  + (str(keys[pos]) +"\t"+ str(dic.get(i,i))+"\n");
        pos+= 1
...
def rgb256x256toDCT(original256x256rgbImagePath):

    """Se realiza la carga de la imagen """
    matrix = Image.open(original256x256rgbImagePath)
    #matrix = Image.open("lena.jpg")
    #matrix = Image.open("great_wall_china.jpeg").resize((256,256))
    # matrix.show()
    ...
    I= Image.fromarray(im2.astype(np.uint8))#.save("./myFolder/ImgFolder/dct.jpg")
    # I = Image.open("./myFolder/ImgFolder/dct.jpg");
    # I.show()
    ...
    for i in range(0,alto):
        for j in range(0,ancho):
            if im2[i,j] == -0.0:
                dctString = dctString + (str(abs(im2[i,j]))+" ")
            else:
                dctString = dctString + (str(im2[i,j])+" ")
        dctString = dctString +("\n")
    ...
    for i in keys:
        ...
        #keys[i];
    probabilityResultTxt = save_probabilidades(probabilidades,keys)
    ...
    return  dctString , probabilityResultTxt , I
